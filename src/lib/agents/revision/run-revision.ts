...
function buildRevisionPrompt(
  ...
  const messagingRules = document_type === 'messaging'
    ? `\n## Outbound Email Rules (Messaging Only)\n\nThese are hard limits enforced by an automated gate. The gate will reject your output if any rule is broken.\n\nWORD COUNTS. Counts include the {{first_name}} line and the sign-off name. They exclude the opt-out footer, which the platform adds at send time. The platform recomputes every count from the text you return, so reporting a flattering number achieves nothing.\nEmail 1 body: ${EMAIL_WORD_LIMITS.email1MinWords} to ${EMAIL_WORD_LIMITS.email1TargetMaxWords} words, hard cap ${EMAIL_WORD_LIMITS.email1MaxWords}. Below ${EMAIL_WORD_LIMITS.email1MinWords} is rejected.\nEmail 2 body: ${EMAIL_WORD_LIMITS.email2MinWords} to ${EMAIL_WORD_LIMITS.email2MaxWords} words, AND no longer than Email 1. Equal lengths pass.\nEmail 3 body: ${EMAIL_WORD_LIMITS.email3MinWords} to ${EMAIL_WORD_LIMITS.email3MaxWords} words, AND no longer than Email 2. Equal lengths pass.\nEmail 4 body: up to ${EMAIL_WORD_LIMITS.email4MaxWords} words. There is no minimum.\n\nIf an email in the CURRENT document already breaks one of these bands, bring it inside the band as part of this revision even if the client's note did not mention that email. The gate validates the whole document, not only the part you changed. Note any such correction in change_summary.\n\nEmail 1 subject_line: maximum ${EMAIL_SUBJECT_LIMITS.email1MaxChars} characters\nEmails 2, 3 and 4 subject_line must be null. All three thread under Email 1, so a reader opening the last email can scroll up and see who is writing. If the current document gives Email 4 a subject, set it to null as part of this revision and note it in change_summary.\n\nEvery email body must end with the sender's first name on its own line (${orgContext.founder_first_name ?? 'the name already in the document'}), nothing after it\n\nThe OPENING line of every email (the first paragraph after {{first_name}}) must not start with "I" or "We". Email 1's third paragraph, which names what changes for the prospect, MAY begin with "We".\n\nNo em dashes, en dashes, or double hyphens anywhere\n\nNo merge tags other than {{first_name}}\n\n## The Email 1 Frame\n\nEmail 1 is a frame of six paragraphs, each with one job. PRESERVE THE SHAPE YOU WERE GIVEN.\n  P1 {{first_name}} on its own line\n  P2 and P3 the observation slot, TWO paragraphs, ONE SENTENCE EACH, each under 25 words:\n     P2 the observation, naming one thing about their situation and nothing else\n     P3 the consequence that follows from it, stated not explained\n     Both are replaced together at send time when prospect research exists, so P2 must\n     stand alone and P3 may refer back to P2. These are the only paragraphs that describe\n     the problem.\n  P4 what changes, the offer line: names a RESULT in the prospect's terms. Never names the service, the mechanism, or features.\n  P5 the CTA question\n  P6 the sign-off\nNo paragraph may restate another. Outside the slot, no paragraph may refer back to the paragraph above it.\n\nSOME DOCUMENTS HAVE A ONE-PARAGRAPH SLOT, written before the slot became two paragraphs.\nBoth shapes are legal and the gate accepts either. DO NOT CHANGE THE NUMBER OF SLOT\nPARAGRAPHS. A revision asked for something else must not merge two slot paragraphs into\none or split one into two: that is a change to the document's shape that nobody requested,\nand the gate cannot tell it apart from a deliberate rewrite.\n\n## Where to Relocate Content That Does Not Fit\n\nIf the client's note asks for more content than Email 1 can hold at ${EMAIL_WORD_LIMITS.email1MaxWords} words:\n- Move the overflow to Email 2 or Email 3 where the word budget allows\n- If the content belongs in a proof point or credentials position: place in Email 4 if it fits within ${EMAIL_WORD_LIMITS.email4MaxWords} words, or note it as a "website or signature suggestion" in change_summary\n\nIf the note asks for a credentials paragraph:\n- That belongs in Email 4 (the breakup position), not Email 1\n- If it makes Email 4 too long: condense the strongest single credential and place it there\n- Note in change_summary what was condensed and why\n`
    : ''
