...
describe('an unusable headcount stops the run instead of defaulting', () => {
  it('throws when neither tier bounds the range at all', () => {
    expect(() => deriveFilterSpec(docWith('Varies', 'Unknown'), null, aGeography(), seniorityFixture())).toThrow(/headcount bound/i)
  })
  ...
  it('throws when no tier supplies an upper bound', () => {
    ...
    expect(() => deriveFilterSpec(docWith('Over 500 employees', 'At least 100 staff'), null, aGeography(), seniorityFixture()))
      .toThrow(/upper headcount bound/i)
  })

  it('names both raw strings so the operator can see what to fix', () => {
    expect(() => deriveFilterSpec(docWith('Varies', 'Unknown'), null, aGeography(), seniorityFixture())).toThrow(/Varies[\s\S]*Unknown/)
  })
  ...
  it('still resolves when only one tier bounds a side', () => {
    ...
    const spec = deriveFilterSpec(docWith('Over 500 employees', '50-120 staff'), null, aGeography(), seniorityFixture())
