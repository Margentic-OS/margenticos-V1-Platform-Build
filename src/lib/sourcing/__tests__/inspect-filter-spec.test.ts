...
describe('handler targeting vs classifier range', () => {
  ...
  //
  // At zero this stops being a ratchet and becomes a plain INVARIANT, so it is asserted
  // as an empty set rather than as a count under a ceiling. A count only ever says how
  // many; the set names them, which is what the next reader needs. Never soften it back
  // into a ceiling without saying why.
  it('targets no industry the classifier cannot produce', async () => {
    ...
    expect(
      unclassifiable,
      'A prospect in one of these is sourced and then removed as industry_off_target ' +
      'with nothing saying why. Add the provider tag to TAG_ALIASES in industry-mapping.ts, ' +
      'which owns that translation.',
    ).toEqual([])
  })

  // THE OTHER DIRECTION, and it is a different failure. The test above catches a name the
  // query can ask for and the classifier cannot label. This one catches a name the
  // classifier can produce and no query can go looking for, which reads to a client as an
  // industry we recognise and can never source.
  it('can search for every industry the classifier is able to produce', async () => {
    ...
    // Same self-guard: an empty result is the correct answer AND the answer a scan that
    // found nothing would give, so these are what stop it passing vacuously.
