...
describe('generation agents carry the rejection note', () => {
  ...
  it.each(agentFiles)('%s records the note on the suggestion it writes', file => {
    // Added 2026-09-03. The note reached the PROMPT and stopped there: the suggestion row
    // carried no note, approve_document_suggestion passed NULL to promote, and every
    // version created that way rendered as "Generated by your outbound team". Five
    // regenerations produced five indistinguishable entries in the version history, which
    // is the one thing that feature exists to prevent.
    //
    // Same family as ADR-038, one step further down the pipe: the note reached something
    // and not the thing that needed it.
    const src = readFileSync(join(AGENT_DIR, file), 'utf8')
    expect(src).toContain('revision_note: noteForVersionHistory(params.regeneration_notes)')
  })
