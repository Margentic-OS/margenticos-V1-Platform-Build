...
Implemented in: pre-c1 cleanup (View-as-Client build)

### Context

The operator needs to navigate the client-facing dashboard scoped to a specific
client's data — for pre-client-zero QA, for sales calls ("here's what your
dashboard will look like"), and for ongoing client support.

Before this ADR, all client-facing routes resolved `organisation_id` from the
authenticated user's own `users` row. There was no mechanism for the operator
to see what a specific client sees. The `?client=` param in the OperatorSidebar
URL only affected the operator sidebar dropdown — it had zero effect on any data
that client routes fetched.

Three approaches were evaluated:

  A) URL-driven scoping: `?client=<orgId>` in the URL drives org resolution on all
     client routes. Middleware injects it as a header so the layout (which cannot
     read searchParams directly in App Router) can resolve the view-as context.

  B) Cookie/session state: clicking "View as" sets a server-side or browser cookie
     recording `viewing_as_org_id`. Requests read it and scope accordingly.

  C) Separate preview route tree: `/dashboard/operator/preview/[clientId]/*`
     shadows all client routes with an explicit `clientId` path param.
...
Security enforcement — two layers, both always active:
  ...
  1. Application layer: `resolveViewingOrg()` (src/lib/dashboard/resolve-viewing-org.ts)
     fetches `role` from the `users` table server-side on every request using the
     authenticated `userId` from the Supabase session. If `role !== 'operator'`,
     the `?client=` param is silently ignored and the user's own `organisation_id`
     is used. The role value is never read from any client-supplied param or header.
     The role check is the first branch — it runs before any code that reads or
     acts on `clientParam`. The function is wrapped in React's `cache()` for
     request-scoped deduplication, ensuring the layout and all pages share a single
     DB round-trip per request without any module-level state that could leak across
     requests.

  2. Database layer: RLS dual-policy pattern (`operators_read_all` / `clients_read_own`)
     is already in place on all tables (ADR-003). A non-operator user cannot read
     another org's rows regardless of what `org_id` the application passes. ADR-003
     three-level isolation is unchanged.

### Reasoning

B rejected: stateful mode creates invisible view-as context that persists across tabs
until explicitly cleared. A URL with no `?client=` param but a scoping cookie looks
correct and isn't. No deep-linking. URL-driven state is self-documenting and
self-cleaning.

C rejected: requires duplicating or abstracting all five client pages and the layout
into a second route tree — significantly more build surface for the same outcome.
Also breaks URL fidelity: the operator sees `/dashboard/operator/preview/abc/pipeline`
rather than `/dashboard/pipeline`, undermining the "see exactly what the client sees"
requirement.

A chosen because: stateless, deep-linkable, URL fidelity preserved, minimal change
surface (12 files), consistent with the existing `x-pathname` middleware injection
...
- Four client-facing pages accept `searchParams` prop (Next.js 15 async Promise) and
  call `resolveViewingOrg()`: `dashboard/page.tsx`, `pipeline/page.tsx`,
  `benchmarks/page.tsx`, `strategy/[type]/page.tsx`.
- `pipeline/page.tsx` redirect for locked pipeline preserves `?client=` param so the
  operator stays in view-as context when the viewed client's pipeline is not yet unlocked.
- `Sidebar.tsx` adds `useSearchParams()` and appends `?client=<id>` to all nav link
  hrefs when the param is present, so navigation within the client experience preserves
  the view-as context.
- `AllClientsView.tsx` "View" button href changed from `/dashboard/operator?client=<id>`
  to `/dashboard?client=<id>` — actually enters the client experience.
- `StrategyPanelCard.tsx` and `DocumentsActiveState.tsx` accept a `clientParam` prop
  and append it to their internal document nav links.
- "Return to operator view" banner link goes to `/dashboard/operator?client=<id>` so
  Doug returns to the operator view with the same client still selected in the sidebar.
- Client users are unaffected. Their routes carry no `?client=` param; even if they
  added one manually, the role check blocks it and RLS backstops it.
- No schema changes. No API route changes. No RLS policy changes.
- Audit logging of view-as actions is deferred (post-c1). Vercel request logs capture
  the `x-view-as-client` header value on every request as an implicit trail.
