...
carries three defects the ICP builder no longer has:

  1. `const buyer = condense(val('clients_clone'), 12)` with NO usability check. Raw
     narrative prose reaches the provider. Its query 2 interpolates `buyer` directly:
     `${buyer} "looking for" OR "need help with" ${service}`, so four of the five live
     organisations send a sentence of someone's story inside a quoted-phrase search.
  2. Geography from CURRENCY, which CLAUDE.md's geography rule forbids and which the ICP
     builder dropped for exactly that reason. `EUR -> 'Europe'` is in all four queries.
  3. No skip path. It cannot decline, so it always sends something.

WHY IT WAS LEFT. The session that fixed the ICP builder was scoped to the ICP builder, and
the two builders are not the same shape: three of the four ICP queries are about the BUYER,
where three of the four positioning queries are about the client's own SERVICE, for which
`company_what_you_do` is the correct input rather than a bug. So only positioning's query 2
needs the buyer-descriptor fix, and all four need the geography fix. Porting it is small
but it is a real behaviour change to a second document type and it wanted its own
before-and-after table rather than being smuggled in.

NEXT ACTION. Port `resolveBuyerDescriptor` and `geographyFromIntake` from
src/agents/icp-generation-agent.ts. They are already exported. Prove it with the same
harness: scripts/prove-research-queries.ts, extended to the positioning builder, with the
"before" column regenerated by scripts/regen-before-research-queries.ts.
...
[phase2] The ICP research builder now falls back to the RECIPIENT named inside
`company_what_you_do` when `clients_clone` is answered as narrative. That is right in four
...
is deliberately not made cleverer.

WHAT WAS DONE INSTEAD. It is made VISIBLE. `ResearchPlan.descriptorNote` names the
population that was researched in suggestion_reason whenever the descriptor came from the
service description rather than from the ideal-client answer, so the operator sees the
wrong population instead of inheriting it silently. That is the whole point of the
2026-08-29 change and it applies to its own fallback too.

THE LONG-TERM ANSWER IS A NEW INTAKE FIELD, and it is logged rather than built because
INTAKE IS WRITE-ONCE WITH NO EDIT PATH. A new field helps future clients and does nothing
for the five that exist, so it could not have been this session's fix. Shape when it is
...
geographic hint at all. Same write-once constraint, same reason it was not fixed here.
...
## SESSION1 ADDENDUM, 2026-08-27: what the client-view verification and the label fix turned up

- [pre-c1] FIVE MORE PLACES A CLIENT IS SHOWN THE INTERNAL DOCUMENT VOCABULARY.

  The dashboard calls them Prospect profile, Positioning, Voice guide and Messaging
  (src/lib/document-labels.ts, whose header already says never to show a client the raw
  value). version-pending.ts and revision-processed.ts were fixed on 2026-08-27. These were
  found in the same sweep and left alone, because each is a PROSE rewrite rather than a
  label swap and the copy has not been reviewed:

    docs-ready.ts:43,45,81,83     "Ideal Client Profile", "Tone of Voice" in the bullet list.
                                  Client-facing. This is the FIFTH vocabulary: "Ideal Client
                                  Profile" appears nowhere in the UI. It also duplicates
                                  DOCUMENT_META[*].desc with different wording.
    list-ready.ts:46,55,86,92     "match your Ideal Client Profile", and "we've nailed your
                                  ICP" sends the bare acronym to a client in body copy.
    IntakeIncompleteState.tsx     :21 desc leaks "ICP definition"; :44 and :109 list "ICP,
                                  positioning, voice guide, and messaging". Lines 19-30 are a
                                  full label-and-desc duplicate of DOCUMENT_META that nobody
                                  spotted because it builds an array rather than a keyed map.
                                  Line 44 also contains an EM DASH, which is its own defect.
    DocumentsActiveState.tsx:126  writes "ICP" by hand at line 126 while the same file
                                  imports DOCUMENT_META and renders meta.label correctly at
                                  line 370.
    FileUploadSection.tsx:21,202  the known "ICP Document", plus line 202 "upload existing
                                  ICP documents", which was missed before. Note PURPOSE_LABELS
                                  is keyed by upload purpose, not document_type, so a
                                  mechanical getDocumentLabel swap does NOT fit it.

  Operator-facing templates keep the internal names on purpose: approval-reminder,
  agent-failure, suggestion-ready, client-revision-notify, revision-gate-failure,
  messaging-revision-staged, all-docs-generated. Recipient verified from each send site.

- [pre-c1] revision-processed.ts HAS NO LINK IN IT AT ALL, and signs off as the client's team.

  Its labels were fixed on 2026-08-27; its copy was not, because the copy has not been
  reviewed. It tells the client to "Log in to your dashboard" and gives no URL, so the
  reader has to go and find the document themselves. That is the same class as the
  version-pending 404: a notification loop nobody has walked. It also signs off
  "<client company> Team", the fault just fixed next door. Fix both together with a
  strategyDocumentUrl deep link and resolvePlatformSender, once the copy is agreed.

- [bug] A CLIENT-ROLE ROUTE APPEARS TO BE BROKEN BY ITS OWN RLS, pre-existing and not on the
  live path today.

  policy clients_read_own_prospects_denied on prospects is FOR SELECT USING (false).
  src/app/api/dashboard/client/prospects/[id]/reject/route.ts runs as the SESSION client
  (role authenticated) and chains .select('id, sourced_tier'), treating zero rows as a 409
  "already sending". Postgres needs SELECT rights to read rows for a WHERE or RETURNING, so a
  genuine client hits a spurious 409.

  MEASURED 2026-08-27, inside BEGIN ... ROLLBACK as a real client via SET LOCAL
  request.jwt.claims: UPDATE prospects ... WHERE id = <their own org's prospect> wrote ZERO
  rows, with auth.uid() and get_my_organisation_id() both resolving correctly and the org
  matching. So clients_update_own_prospect_review is effectively dead for any UPDATE that
  reads, which is all of them.

  NOT URGENT: the mounted client UI uses the service-role routes
  (/api/dashboard/client/prospects/reject and /approve-all), and the component that calls the
  broken route, TierCard.tsx, has no importer anywhere. No test covers any of the five client
  prospect routes, which is why this has never surfaced.

- [monitor] MON-024 could be tightened once the anon revoke happens. It currently accepts any
