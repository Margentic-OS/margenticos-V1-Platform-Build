...
-- Captured 2026-09-15 20:47 UTC from the LIVE database, read-only.
...
CREATE TABLE IF NOT EXISTS public.meetings (
  ...
  outcome_requested_at timestamp with time zone,
  confirmation_sent_at timestamp with time zone,
  last_reminded_at timestamp with time zone,
  reminder_count integer DEFAULT 0 NOT NULL,
  bill_unconfirmed_after timestamp with time zone,
  billable_basis text
);
...
CREATE TABLE IF NOT EXISTS public.organisations (
  ...
  sourcing_revenue_filter_enabled boolean DEFAULT false NOT NULL,
  booking_url text,
  booking_host_ref text
);
...
CREATE TABLE IF NOT EXISTS public.prospects (
  ...
  personalisation_subject text,
  sourcing_run_id uuid,
  outbound_suppression_status text,
  outbound_suppression_at timestamp with time zone,
  outbound_suppression_error text,
  send_hold_at timestamp with time zone,
  send_hold_by uuid,
...
CREATE TABLE IF NOT EXISTS public.reply_reconciliation_snapshot (
  ...
  campaigns_checked integer DEFAULT 0 NOT NULL,
  provider_reply_count integer DEFAULT 0 NOT NULL,
  stored_reply_count integer DEFAULT 0 NOT NULL,
  missing_count integer DEFAULT 0 NOT NULL,
  unreachable_campaigns integer DEFAULT 0 NOT NULL,
  incomplete boolean DEFAULT false NOT NULL,
  missing_sample text[] DEFAULT '{}'::text[] NOT NULL,
  detail text,
...
CREATE TABLE IF NOT EXISTS public.sourcing_runs (
  ...
  candidates_returned integer DEFAULT 0 NOT NULL,
  prospects_written integer DEFAULT 0 NOT NULL,
  dropped_by_reason jsonb DEFAULT '{}'::jsonb NOT NULL,
  error_message text,
...
CREATE TABLE IF NOT EXISTS public.suppressed_emails (
  ...
  revoked_reason text,
  carry_status text,
  carry_attempted_at timestamp with time zone,
  carry_error text
);
...
CREATE TABLE IF NOT EXISTS public.suppression_reconciliation_snapshot (
  ...
  incomplete boolean DEFAULT false NOT NULL,
  unreconciled_prospect_ids jsonb DEFAULT '[]'::jsonb NOT NULL,
  detail text NOT NULL,
  computed_at timestamp with time zone DEFAULT now() NOT NULL,
  uncarried_count integer DEFAULT 0 NOT NULL,
  carry_failed_count integer DEFAULT 0 NOT NULL
);
...
CREATE TABLE IF NOT EXISTS public.tuning_rounds (
  ...
  billable_searches integer DEFAULT 0 NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.tuning_runs (
  ...
  icp_document_updated_at timestamp with time zone,
  terminal_state text,
  terminal_reason text,
  baseline_population integer,
  ceiling_population integer,
  rounds_run integer DEFAULT 0 NOT NULL,
  model_calls integer DEFAULT 0 NOT NULL,
  billable_searches integer DEFAULT 0 NOT NULL,
  provider_calls integer DEFAULT 0 NOT NULL,
  plan jsonb,
  ...
  created_at timestamp with time zone DEFAULT now() NOT NULL,
  sample_size integer,
  fit_sampled_rows integer,
  fit_resolved_rows integer,
  fit_of_resolved numeric,
  fit_interval_low numeric,
  fit_interval_high numeric,
  rubric_source text
);
...
CREATE TABLE IF NOT EXISTS public.unattributed_replies (
  ...
  body_redacted_at timestamp with time zone,
  resolved_at timestamp with time zone,
  resolved_signal_id uuid,
  reply_written_at timestamp with time zone
);
...
ALTER TABLE public.cron_schedule_registry ADD CONSTRAINT cron_schedule_registry_pkey PRIMARY KEY (jobname);
ALTER TABLE public.dashboard_failures ADD CONSTRAINT dashboard_failures_kind_check CHECK ((kind = ANY (ARRAY['read'::text, 'render'::text])));
ALTER TABLE public.dashboard_failures ADD CONSTRAINT dashboard_failures_pkey PRIMARY KEY (id);
...
ALTER TABLE public.email_delivery_failures ADD CONSTRAINT email_delivery_failures_audience_check CHECK ((audience = ANY (ARRAY['operator'::text, 'customer'::text])));
ALTER TABLE public.email_delivery_failures ADD CONSTRAINT email_delivery_failures_pkey PRIMARY KEY (id);
ALTER TABLE public.email_delivery_failures ADD CONSTRAINT email_delivery_failures_stage_check CHECK ((stage = ANY (ARRAY['content_validation'::text, 'provider_send'::text, 'recipient_refused'::text])));
...
ALTER TABLE public.gateway_retry_counts ADD CONSTRAINT gateway_retry_counts_method_check CHECK ((method = ANY (ARRAY['GET'::text, 'HEAD'::text, 'OPTIONS'::text])));
ALTER TABLE public.gateway_retry_counts ADD CONSTRAINT gateway_retry_counts_pkey PRIMARY KEY (day, method);
ALTER TABLE public.gateway_retry_counts ADD CONSTRAINT gateway_retry_counts_retries_check CHECK ((retries >= 0));
...
ALTER TABLE public.meetings ADD CONSTRAINT meetings_billable_basis_check CHECK (((billable_basis IS NULL) OR (billable_basis = ANY (ARRAY['client_confirmed'::text, 'operator_marked'::text, 'unconfirmed_backstop'::text]))));
ALTER TABLE public.meetings ADD CONSTRAINT meetings_billable_needs_a_prospect CHECK (((NOT is_billable) OR (prospect_id IS NOT NULL)));
ALTER TABLE public.meetings ADD CONSTRAINT meetings_billable_records_its_basis CHECK (((NOT is_billable) OR (billable_basis IS NOT NULL)));
...
ALTER TABLE public.meetings ADD CONSTRAINT meetings_held_confirmed_by_check CHECK (((held_confirmed_by IS NULL) OR (held_confirmed_by = ANY (ARRAY['client'::text, 'operator'::text, 'auto'::text, 'host'::text]))));
...
ALTER TABLE public.meetings ADD CONSTRAINT meetings_prospect_match_check CHECK (((prospect_match IS NULL) OR (prospect_match = ANY (ARRAY['link'::text, 'email'::text, 'none'::text]))));
...
ALTER TABLE public.meetings ADD CONSTRAINT meetings_source_check CHECK ((source = ANY (ARRAY['calendly'::text, 'manual'::text, 'webhook'::text])));
...
ALTER TABLE public.organisations ADD CONSTRAINT organisations_booking_host_ref_key UNIQUE (booking_host_ref);
ALTER TABLE public.organisations ADD CONSTRAINT organisations_booking_host_ref_normalised CHECK (((booking_host_ref IS NULL) OR (booking_host_ref = lower(btrim(booking_host_ref)))));
...
ALTER TABLE public.prospect_research_results ADD CONSTRAINT prospect_research_results_icp_fit_check CHECK ((icp_fit = ANY (ARRAY['strong'::text, 'moderate'::text, 'weak'::text, 'unassessed'::text, 'cannot_tell'::text])));
...
ALTER TABLE public.prospects ADD CONSTRAINT prospects_icp_fit_check CHECK ((icp_fit = ANY (ARRAY['strong'::text, 'moderate'::text, 'weak'::text, 'unassessed'::text, 'cannot_tell'::text])));
...
ALTER TABLE public.prospects ADD CONSTRAINT prospects_outbound_suppression_error_complete CHECK (((outbound_suppression_error IS NULL) OR (outbound_suppression_status = 'failed'::text)));
ALTER TABLE public.prospects ADD CONSTRAINT prospects_outbound_suppression_status_valid CHECK (((outbound_suppression_status IS NULL) OR (outbound_suppression_status = ANY (ARRAY['not_required'::text, 'confirmed'::text, 'failed'::text]))));
...
ALTER TABLE public.prospects ADD CONSTRAINT prospects_send_hold_complete CHECK ((((send_hold_at IS NULL) AND (send_hold_reason IS NULL)) OR ((send_hold_at IS NOT NULL) AND (send_hold_reason IS NOT NULL))));
...
ALTER TABLE public.reply_draft_ageing_config ADD CONSTRAINT reply_draft_ageing_config_pkey PRIMARY KEY (id);
ALTER TABLE public.reply_draft_ageing_config ADD CONSTRAINT reply_draft_ageing_config_threshold_hours_check CHECK ((threshold_hours > 0));
ALTER TABLE public.reply_drafts ADD CONSTRAINT reply_drafts_body_required CHECK ((((status = ANY (ARRAY['manual_required'::text, 'draft_failed'::text])) AND (ai_draft_body IS NULL)) OR ((status = ANY (ARRAY['pending'::text, 'approved'::text, 'sent'::text, 'send_failed'::text])) AND (ai_draft_body IS NOT NULL)) OR (status = 'rejected'::text)));
...
ALTER TABLE public.reply_reconciliation_snapshot ADD CONSTRAINT reply_reconciliation_snapshot_id_check CHECK ((id = 1));
ALTER TABLE public.reply_reconciliation_snapshot ADD CONSTRAINT reply_reconciliation_snapshot_pkey PRIMARY KEY (id);
...
ALTER TABLE public.sourcing_runs ADD CONSTRAINT sourcing_runs_live_rows_know_their_target CHECK (((backfilled_at IS NOT NULL) OR (target_batch_size IS NOT NULL)));
ALTER TABLE public.sourcing_runs ADD CONSTRAINT sourcing_runs_pkey PRIMARY KEY (id);
ALTER TABLE public.sourcing_runs ADD CONSTRAINT sourcing_runs_status_check CHECK ((status = ANY (ARRAY['running'::text, 'completed'::text, 'failed'::text])));
...
ALTER TABLE public.strategy_documents ADD CONSTRAINT strategy_documents_update_trigger_check CHECK ((update_trigger = ANY (ARRAY['initial'::text, 'signal_suggestion'::text, 'intake_update'::text, 'manual'::text, 'client_revision'::text, 'revert'::text])));
ALTER TABLE public.suppressed_emails ADD CONSTRAINT suppressed_emails_carry_complete CHECK ((((carry_status IS NULL) AND (carry_attempted_at IS NULL)) OR ((carry_status IS NOT NULL) AND (carry_attempted_at IS NOT NULL))));
ALTER TABLE public.suppressed_emails ADD CONSTRAINT suppressed_emails_carry_error_matches_status CHECK ((((carry_status = 'failed'::text) AND (carry_error IS NOT NULL) AND (length(btrim(carry_error)) > 0)) OR ((carry_status IS DISTINCT FROM 'failed'::text) AND (carry_error IS NULL))));
ALTER TABLE public.suppressed_emails ADD CONSTRAINT suppressed_emails_carry_status_valid CHECK (((carry_status IS NULL) OR (carry_status = ANY (ARRAY['not_required'::text, 'confirmed'::text, 'failed'::text]))));
...
ALTER TABLE public.suppression_reconciliation_snapshot ADD CONSTRAINT suppression_reconciliation_snapshot_id_check CHECK ((id = 1));
ALTER TABLE public.suppression_reconciliation_snapshot ADD CONSTRAINT suppression_reconciliation_snapshot_pkey PRIMARY KEY (id);
...
ALTER TABLE public.tuning_rounds ADD CONSTRAINT tuning_rounds_kind_check CHECK ((kind = ANY (ARRAY['zero'::text, 'adjust'::text])));
ALTER TABLE public.tuning_rounds ADD CONSTRAINT tuning_rounds_pkey PRIMARY KEY (id);
ALTER TABLE public.tuning_rounds ADD CONSTRAINT tuning_rounds_run_id_round_index_key UNIQUE (run_id, round_index);
ALTER TABLE public.tuning_runs ADD CONSTRAINT tuning_runs_operator_verdict_check CHECK ((operator_verdict = ANY (ARRAY['agree'::text, 'disagree'::text, 'partly'::text])));
ALTER TABLE public.tuning_runs ADD CONSTRAINT tuning_runs_pkey PRIMARY KEY (id);
ALTER TABLE public.tuning_runs ADD CONSTRAINT tuning_runs_rubric_source_check CHECK (((rubric_source IS NULL) OR (rubric_source = ANY (ARRAY['stored_conditions'::text, 'derived_this_run'::text]))));
ALTER TABLE public.tuning_runs ADD CONSTRAINT tuning_runs_terminal_state_check CHECK ((terminal_state = ANY (ARRAY['accepted'::text, 'population_too_small_for_contract'::text, 'population_effectively_empty'::text, 'no_taxonomy_bucket_fits'::text, 'document_unresolved_fields_block_search'::text, 'no_round_improved'::text, 'judge_unreliable'::text, 'name_signal_too_low'::text, 'wall_clock_exhausted'::text, 'lookup_budget_exhausted'::text, 'provider_ignored_a_parameter'::text, 'rate_limit_reached'::text, 'forbidden_change_required'::text, 'failed'::text, 'audience_cannot_support_the_work'::text, 'no_combination_reached_the_standard'::text, 'derivation_refused'::text, 'spend_cap_reached'::text, 'taxonomy_cannot_express_the_document'::text])));
ALTER TABLE public.tuning_runs ADD CONSTRAINT tuning_runs_trigger_type_check CHECK ((trigger_type = ANY (ARRAY['operator_manual'::text, 'instruction'::text])));
ALTER TABLE public.unattributed_bookings ADD CONSTRAINT unattributed_bookings_pkey PRIMARY KEY (id);
ALTER TABLE public.unattributed_replies ADD CONSTRAINT unattributed_replies_pkey PRIMARY KEY (id);
...
ALTER TABLE public.dashboard_failures ADD CONSTRAINT dashboard_failures_organisation_id_fkey FOREIGN KEY (organisation_id) REFERENCES organisations(id) ON DELETE SET NULL;
...
ALTER TABLE public.prospects ADD CONSTRAINT prospects_sourcing_run_id_fkey FOREIGN KEY (sourcing_run_id) REFERENCES sourcing_runs(id) ON DELETE RESTRICT;
...
ALTER TABLE public.reply_draft_ageing_config ADD CONSTRAINT reply_draft_ageing_config_organisation_id_fkey FOREIGN KEY (organisation_id) REFERENCES organisations(id) ON DELETE CASCADE;
...
ALTER TABLE public.sourcing_runs ADD CONSTRAINT sourcing_runs_agent_run_id_fkey FOREIGN KEY (agent_run_id) REFERENCES agent_runs(id);
ALTER TABLE public.sourcing_runs ADD CONSTRAINT sourcing_runs_created_by_fkey FOREIGN KEY (created_by) REFERENCES users(id);
ALTER TABLE public.sourcing_runs ADD CONSTRAINT sourcing_runs_icp_document_id_fkey FOREIGN KEY (icp_document_id) REFERENCES strategy_documents(id);
ALTER TABLE public.sourcing_runs ADD CONSTRAINT sourcing_runs_organisation_id_fkey FOREIGN KEY (organisation_id) REFERENCES organisations(id);
...
ALTER TABLE public.tuning_rounds ADD CONSTRAINT tuning_rounds_run_id_fkey FOREIGN KEY (run_id) REFERENCES tuning_runs(id) ON DELETE CASCADE;
ALTER TABLE public.tuning_runs ADD CONSTRAINT tuning_runs_icp_document_id_fkey FOREIGN KEY (icp_document_id) REFERENCES strategy_documents(id) ON DELETE SET NULL;
ALTER TABLE public.tuning_runs ADD CONSTRAINT tuning_runs_organisation_id_fkey FOREIGN KEY (organisation_id) REFERENCES organisations(id) ON DELETE CASCADE;
ALTER TABLE public.unattributed_bookings ADD CONSTRAINT unattributed_bookings_resolved_meeting_id_fkey FOREIGN KEY (resolved_meeting_id) REFERENCES meetings(id) ON DELETE SET NULL;
ALTER TABLE public.unattributed_replies ADD CONSTRAINT unattributed_replies_resolved_signal_id_fkey FOREIGN KEY (resolved_signal_id) REFERENCES signals(id) ON DELETE SET NULL;
...
CREATE INDEX dashboard_failures_occurred_at_idx ON public.dashboard_failures USING btree (occurred_at DESC);
...
CREATE INDEX email_delivery_failures_outstanding ON public.email_delivery_failures USING btree (created_at DESC) WHERE (resolved_at IS NULL);
...
CREATE INDEX idx_prospects_sourcing_run ON public.prospects USING btree (organisation_id, sourcing_run_id);
...
CREATE INDEX idx_sourcing_runs_org_started ON public.sourcing_runs USING btree (organisation_id, started_at DESC);
...
CREATE INDEX meetings_awaiting_outcome_idx ON public.meetings USING btree (scheduled_end_at) WHERE ((meeting_status = 'booked'::text) AND (held_decision_locked = false));
CREATE INDEX meetings_billable_by_org_idx ON public.meetings USING btree (organisation_id, billable_basis) WHERE (is_billable = true);
CREATE INDEX meetings_billing_deadline_idx ON public.meetings USING btree (bill_unconfirmed_after) WHERE ((meeting_status = 'booked'::text) AND (held_decision_locked = false));
...
CREATE INDEX prospects_outbound_suppression_unfinished_idx ON public.prospects USING btree (organisation_id) WHERE ((suppressed = true) AND ((outbound_suppression_status IS NULL) OR (outbound_suppression_status = 'failed'::text)));
CREATE INDEX prospects_send_hold_at_idx ON public.prospects USING btree (send_hold_at) WHERE (send_hold_at IS NOT NULL);
CREATE INDEX reply_actions_link_sent_at_idx ON public.reply_handling_actions USING btree (link_sent_at) WHERE (link_sent_at IS NOT NULL);
CREATE UNIQUE INDEX reply_draft_ageing_config_default_singleton ON public.reply_draft_ageing_config USING btree ((true)) WHERE (organisation_id IS NULL);
CREATE UNIQUE INDEX reply_draft_ageing_config_one_per_org ON public.reply_draft_ageing_config USING btree (organisation_id) WHERE (organisation_id IS NOT NULL);
...
CREATE INDEX suppressed_emails_needs_carry_idx ON public.suppressed_emails USING btree (carry_attempted_at NULLS FIRST) WHERE ((revoked_at IS NULL) AND ((carry_status IS NULL) OR (carry_status = 'failed'::text)));
...
CREATE INDEX tuning_rounds_run_idx ON public.tuning_rounds USING btree (run_id, round_index);
CREATE INDEX tuning_runs_org_started_idx ON public.tuning_runs USING btree (organisation_id, started_at DESC);
CREATE UNIQUE INDEX unattributed_bookings_provider_uid_key ON public.unattributed_bookings USING btree (provider, provider_booking_uid);
CREATE UNIQUE INDEX unattributed_replies_provider_email_id_key ON public.unattributed_replies USING btree (provider, provider_email_id);
CREATE INDEX unattributed_replies_unresolved_age_idx ON public.unattributed_replies USING btree (first_seen_at) WHERE (resolved_at IS NULL);
CREATE INDEX unattributed_replies_unresolved_campaign_idx ON public.unattributed_replies USING btree (provider_campaign_id) WHERE (resolved_at IS NULL);
...
CREATE OR REPLACE FUNCTION public.approve_document_suggestion(p_suggestion_id uuid, p_reviewer_id uuid, p_plain_text text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.promote_strategy_doc_version(p_org_id uuid, p_doc_type text, p_segment_id uuid, p_content jsonb, p_update_trigger text, p_revision_note text DEFAULT NULL::text, p_change_summary text DEFAULT NULL::text, p_plain_text text DEFAULT NULL::text, p_generated_by_model text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE VIEW public.mon_001 AS
 SELECT 'MON-001'::text AS check_code,
        CASE
            WHEN sw.live_active IS FALSE AND sw.declared_active IS FALSE AND (latest.ran_at IS NULL OR (EXTRACT(epoch FROM now() - latest.ran_at) / 60::numeric) > 75::numeric) THEN 'OK'::text
            WHEN sw.live_active IS FALSE AND sw.declared_active IS FALSE THEN 'PROBLEM'::text
            WHEN sw.live_active IS FALSE THEN 'PROBLEM'::text
            WHEN latest.ran_at IS NULL THEN 'UNKNOWN'::text
            WHEN (EXTRACT(epoch FROM now() - latest.ran_at) / 60::numeric) > 75::numeric THEN 'PROBLEM'::text
            WHEN latest.ok = false THEN 'PROBLEM'::text
            ELSE 'OK'::text
        END AS state,
        CASE
            WHEN sw.live_active IS FALSE AND sw.declared_active IS FALSE AND (latest.ran_at IS NULL OR (EXTRACT(epoch FROM now() - latest.ran_at) / 60::numeric) > 75::numeric) THEN ((('Switched off, as declared: the pg_cron job is inactive and '::text || sw.declared_by) || ' declares it off, so no run is expected. Last run '::text) || COALESCE(to_char(latest.ran_at, 'YYYY-MM-DD HH24:MI:SS UTC'::text), 'never'::text)) || '.'::text
            WHEN sw.live_active IS FALSE AND sw.declared_active IS FALSE THEN ('Declared off and inactive in pg_cron, yet it reported a run at '::text || to_char(latest.ran_at, 'YYYY-MM-DD HH24:MI:SS UTC'::text)) || '. Something other than its schedule is calling the route. ADR-052 says this job must not run.'::text
            WHEN sw.live_active IS FALSE THEN (('The pg_cron job is switched off and nothing declares it off, so this is not '::text || 'treated as deliberate. Declare it off in a migration, or switch it back on. Last run '::text) || COALESCE(to_char(latest.ran_at, 'YYYY-MM-DD HH24:MI:SS UTC'::text), 'never'::text)) || '.'::text
            WHEN latest.ran_at IS NULL THEN 'auto-approve has never reported. Check the pg_cron job exists and is active.'::text
            WHEN (EXTRACT(epoch FROM now() - latest.ran_at) / 60::numeric) > 75::numeric THEN ('Last run '::text || to_char(latest.ran_at, 'YYYY-MM-DD HH24:MI:SS UTC'::text)) || ', over 75 minutes ago.'::text
   ...
   FROM ( SELECT 1 AS "?column?") one
     CROSS JOIN ( SELECT ( SELECT bool_or(j.active) AS bool_or
                   FROM cron.job j
                  WHERE j.jobname = 'auto-approve'::text) AS live_active,
            ( SELECT r.active
                   FROM cron_schedule_registry r
                  WHERE r.jobname = 'auto-approve'::text) AS declared_active,
            ( SELECT r.declared_by
                   FROM cron_schedule_registry r
                  WHERE r.jobname = 'auto-approve'::text) AS declared_by) sw
     LEFT JOIN LATERAL ( SELECT h.ran_at,
            h.ok,
            h.detail
           FROM cron_heartbeats h
          WHERE h.job_name = 'auto-approve'::text
...
CREATE OR REPLACE VIEW public.mon_002 AS
 SELECT 'MON-002'::text AS check_code,
        ...
        END AS state,
        CASE
            WHEN latest.ran_at IS NULL THEN 'instantly-poll has never reported. Check the pg_cron job exists and is active.'::text
            WHEN (EXTRACT(epoch FROM now() - latest.ran_at) / 60::numeric) > 30::numeric THEN ('Last run '::text || to_char(latest.ran_at, 'YYYY-MM-DD HH24:MI:SS UTC'::text)) || ', over 30 minutes ago.'::text
            WHEN latest.ok = false THEN 'Last run FAILED: '::text || COALESCE(latest.detail, 'no detail'::text)
            ELSE 'Last run OK: '::text || COALESCE(latest.detail, to_char(latest.ran_at, 'YYYY-MM-DD HH24:MI:SS UTC'::text))
        END AS detail,
    latest.ran_at AS last_run
...
CREATE OR REPLACE VIEW public.mon_003 AS
 SELECT 'MON-003'::text AS check_code,
        ...
        END AS state,
        CASE
            WHEN latest.ran_at IS NULL THEN 'process-replies has never reported. Check the pg_cron job exists and is active.'::text
            WHEN (EXTRACT(epoch FROM now() - latest.ran_at) / 60::numeric) > 10::numeric THEN ('Last run '::text || to_char(latest.ran_at, 'YYYY-MM-DD HH24:MI:SS UTC'::text)) || ', over 10 minutes ago.'::text
            WHEN latest.ok = false THEN 'Last run FAILED: '::text || COALESCE(latest.detail, 'no detail'::text)
            ELSE 'Last run OK: '::text || COALESCE(latest.detail, to_char(latest.ran_at, 'YYYY-MM-DD HH24:MI:SS UTC'::text))
        END AS detail,
    latest.ran_at AS last_run
...
CREATE OR REPLACE VIEW public.mon_004 AS
 SELECT 'MON-004'::text AS check_code,
        CASE
            WHEN latest.ran_at IS NULL THEN 'UNKNOWN'::text
            WHEN (EXTRACT(epoch FROM now() - latest.ran_at) / 60::numeric) > 20::numeric THEN 'PROBLEM'::text
            WHEN latest.ok = false THEN 'PROBLEM'::text
            ELSE 'OK'::text
        END AS state,
        CASE
            WHEN latest.ran_at IS NULL THEN 'reap-agent-runs has never reported. Check the pg_cron job exists and is active.'::text
            WHEN (EXTRACT(epoch FROM now() - latest.ran_at) / 60::numeric) > 20::numeric THEN ('Last run '::text || to_char(latest.ran_at, 'YYYY-MM-DD HH24:MI:SS UTC'::text)) || ', over 20 minutes ago.'::text
            WHEN latest.ok = false THEN 'Last run FAILED: '::text || COALESCE(latest.detail, 'no detail'::text)
            ELSE 'Last run OK: '::text || COALESCE(latest.detail, to_char(latest.ran_at, 'YYYY-MM-DD HH24:MI:SS UTC'::text))
        END AS detail,
    latest.ran_at AS last_run
   FROM ( SELECT 1 AS "?column?") one
     LEFT JOIN LATERAL ( SELECT h.ran_at,
            ...
            h.detail
           FROM cron_heartbeats h
          WHERE h.job_name = 'reap-agent-runs'::text
          ORDER BY h.ran_at DESC, h.id DESC
         LIMIT 1) latest ON true;
...
CREATE OR REPLACE VIEW public.mon_005 AS
 SELECT 'MON-005'::text AS check_code,
        ...
        END AS state,
        CASE
            WHEN latest.ran_at IS NULL THEN 'monitor-sweep has never reported. Check the pg_cron job exists and is active.'::text
            WHEN (EXTRACT(epoch FROM now() - latest.ran_at) / 60::numeric) > 30::numeric THEN ('Last run '::text || to_char(latest.ran_at, 'YYYY-MM-DD HH24:MI:SS UTC'::text)) || ', over 30 minutes ago.'::text
            WHEN latest.ok = false THEN 'Last run FAILED: '::text || COALESCE(latest.detail, 'no detail'::text)
            ELSE 'Last run OK: '::text || COALESCE(latest.detail, to_char(latest.ran_at, 'YYYY-MM-DD HH24:MI:SS UTC'::text))
        END AS detail,
   ...
   FROM ( SELECT 1 AS "?column?") one
     LEFT JOIN LATERAL ( SELECT h.ran_at,
            ...
            h.detail
           FROM cron_heartbeats h
          WHERE h.job_name = 'monitor-sweep'::text
          ORDER BY h.ran_at DESC, h.id DESC
         LIMIT 1) latest ON true;
...
CREATE OR REPLACE VIEW public.mon_010 AS
 SELECT 'MON-010'::text AS check_code,
        ...
        CASE
            ...
            WHEN (EXTRACT(epoch FROM now() - ran_at) / 60::numeric) > 1500::numeric THEN ('Last run '::text || to_char(ran_at, 'YYYY-MM-DD HH24:MI:SS UTC'::text)) || ', over 1500 minutes ago.'::text
            WHEN ok = false THEN 'Last run FAILED: '::text || COALESCE(detail, 'no detail'::text)
            WHEN examined IS NULL THEN 'Cannot verify. Heartbeat detail does not match the expected "Examined N organisations" format, so the organisation cross-check could not run. Reporting UNKNOWN rather than OK. Detail was: '::text || COALESCE(detail, 'no detail'::text)
            WHEN examined < org_count THEN ((((('Run reported ok but examined only '::text || examined::text) || ' organisation(s) while '::text) || org_count::text) || ' existed at '::text) || to_char(ran_at, 'YYYY-MM-DD HH24:MI:SS UTC'::text)) || '. The job is reporting success over work it did not do. Check the cron is using a service-role client: this is exactly how resolve-auto-held ran as anon for a month.'::text
...
CREATE OR REPLACE VIEW public.mon_013 AS
 WITH orphaned AS (
         SELECT count(*) AS count,
          ...
          WHERE c.external_id IS NULL AND o.archived_at IS NULL
        )
 SELECT 'MON-013'::text AS check_code,
        ...
        END AS state,
        CASE
            WHEN count = 0 THEN 'Every campaign of a live client has a provider id.'::text
            ELSE (((((count::text || ' campaign(s) have no provider id, so they cannot be polled, '::text) || 'reconciled, or matched to an incoming reply. First: '::text) || COALESCE(first_name, '(unnamed)'::text)) || ', status '::text) || COALESCE(first_status, '(none)'::text)) || '.'::text
        END AS detail,
    now() AS check_time
   FROM orphaned;
...
CREATE OR REPLACE VIEW public.mon_019 AS
 SELECT 'MON-019'::text AS check_code,
        CASE
            WHEN agg.last_run IS NULL THEN 'UNKNOWN'::text
            WHEN (EXTRACT(epoch FROM now() - agg.last_run) / 60::numeric) > 20::numeric THEN 'PROBLEM'::text
            WHEN agg.recent_failure THEN 'PROBLEM'::text
            ELSE 'OK'::text
        END AS state,
        CASE
            WHEN agg.last_run IS NULL THEN 'verify-pending has never reported. Check the pg_cron job exists and is active.'::text
            WHEN (EXTRACT(epoch FROM now() - agg.last_run) / 60::numeric) > 20::numeric THEN ('Last run '::text || to_char(agg.last_run, 'YYYY-MM-DD HH24:MI:SS UTC'::text)) || ', over 20 minutes ago.'::text
            WHEN agg.recent_failure THEN (('Run at '::text || to_char(fail.ran_at, 'YYYY-MM-DD HH24:MI:SS UTC'::text)) || ' FAILED: '::text) || COALESCE(fail.detail, 'no detail'::text)
            ELSE 'Last run: '::text || to_char(agg.last_run, 'YYYY-MM-DD HH24:MI:SS UTC'::text)
        END AS detail,
    agg.last_run
   FROM ( SELECT max(h.ran_at) AS last_run,
            bool_or(NOT h.ok) FILTER (WHERE h.ran_at > (now() - '01:00:00'::interval)) AS recent_failure
           FROM cron_heartbeats h
          WHERE h.job_name = 'verify-pending'::text) agg
     LEFT JOIN LATERAL ( SELECT h.ran_at,
            h.detail
           FROM cron_heartbeats h
          WHERE h.job_name = 'verify-pending'::text AND NOT h.ok AND h.ran_at > (now() - '01:00:00'::interval)
          ORDER BY h.ran_at DESC, h.id DESC
         LIMIT 1) fail ON true;

CREATE OR REPLACE VIEW public.mon_020 AS
 SELECT 'MON-020'::text AS check_code,
        CASE
            WHEN agg.last_run IS NULL THEN 'UNKNOWN'::text
            WHEN (EXTRACT(epoch FROM now() - agg.last_run) / 60::numeric) > 90::numeric THEN 'PROBLEM'::text
            WHEN agg.recent_failure THEN 'PROBLEM'::text
            ELSE 'OK'::text
        END AS state,
        CASE
            WHEN agg.last_run IS NULL THEN 'verify-catch-all has never reported. Check the pg_cron job exists and is active.'::text
            WHEN (EXTRACT(epoch FROM now() - agg.last_run) / 60::numeric) > 90::numeric THEN ('Last run '::text || to_char(agg.last_run, 'YYYY-MM-DD HH24:MI:SS UTC'::text)) || ', over 90 minutes ago.'::text
            WHEN agg.recent_failure THEN (('Run at '::text || to_char(fail.ran_at, 'YYYY-MM-DD HH24:MI:SS UTC'::text)) || ' FAILED: '::text) || COALESCE(fail.detail, 'no detail'::text)
            ELSE 'Last run: '::text || to_char(agg.last_run, 'YYYY-MM-DD HH24:MI:SS UTC'::text)
        END AS detail,
    agg.last_run
   FROM ( SELECT max(h.ran_at) AS last_run,
            bool_or(NOT h.ok) FILTER (WHERE h.ran_at > (now() - '02:00:00'::interval)) AS recent_failure
           FROM cron_heartbeats h
          WHERE h.job_name = 'verify-catch-all'::text) agg
     LEFT JOIN LATERAL ( SELECT h.ran_at,
            h.detail
           FROM cron_heartbeats h
          WHERE h.job_name = 'verify-catch-all'::text AND NOT h.ok AND h.ran_at > (now() - '02:00:00'::interval)
          ORDER BY h.ran_at DESC, h.id DESC
         LIMIT 1) fail ON true;

CREATE OR REPLACE VIEW public.mon_021 AS
 WITH hb AS (
         SELECT max(h.ran_at) AS last_run,
            bool_or(NOT h.ok) FILTER (WHERE h.ran_at > (now() - '01:00:00'::interval)) AS recent_failure
           FROM cron_heartbeats h
          WHERE h.job_name = 'synthesis-batch-sweep'::text
        ), hb_fail AS (
         SELECT h.ran_at,
            h.detail
           FROM cron_heartbeats h
          WHERE h.job_name = 'synthesis-batch-sweep'::text AND NOT h.ok AND h.ran_at > (now() - '01:00:00'::interval)
          ORDER BY h.ran_at DESC, h.id DESC
         LIMIT 1
        ), batches AS (
         SELECT count(*) FILTER (WHERE b.state = 'attempted'::text AND b.requested_at < (now() - '00:30:00'::interval)) AS unreceipted,
            count(*) FILTER (WHERE (b.state = ANY (ARRAY['submitted'::text, 'ended'::text])) AND b.requested_at < (now() - '26:00:00'::interval)) AS overdue,
            max(EXTRACT(epoch FROM now() - b.requested_at) / 3600::numeric) FILTER (WHERE b.state = ANY (ARRAY['attempted'::text, 'submitted'::text, 'ended'::text])) AS oldest_open_hours
           FROM synthesis_batches b
        ), entries AS (
         SELECT count(*) FILTER (WHERE e.state = 'pending_submission'::text AND e.created_at < (now() - '00:30:00'::interval)) AS stuck_pending,
            count(*) FILTER (WHERE (e.state = ANY (ARRAY['succeeded'::text, 'collected'::text])) AND e.updated_at > (now() - '24:00:00'::interval)) AS good_24h,
            count(*) FILTER (WHERE (e.state = ANY (ARRAY['errored'::text, 'expired'::text, 'cancelled'::text, 'failed'::text])) AND e.updated_at > (now() - '24:00:00'::interval)) AS bad_24h,
            count(*) FILTER (WHERE e.doc_superseded AND e.updated_at > (now() - '24:00:00'::interval)) AS superseded_24h
           FROM synthesis_batch_entries e
        )
 SELECT 'MON-021'::text AS check_code,
        ...
        CASE
            ...
            WHEN hb.recent_failure THEN (('Run at '::text || to_char(hb_fail.ran_at, 'YYYY-MM-DD HH24:MI:SS UTC'::text)) || ' FAILED: '::text) || COALESCE(hb_fail.detail, 'no detail'::text)
...
CREATE OR REPLACE VIEW public.mon_025 AS
 WITH live AS (
         ...
         SELECT cron_schedule_registry.jobname,
            cron_schedule_registry.schedule,
            cron_schedule_registry.active
           FROM cron_schedule_registry
        ), findings AS (
         SELECT ( SELECT string_agg((((d.jobname || ' runs '::text) || l.schedule) || ', declared '::text) || d.schedule, ', '::text ORDER BY d.jobname) AS string_agg
                   FROM declared d
                     JOIN live l USING (jobname)
                  WHERE l.schedule IS DISTINCT FROM d.schedule) AS mismatched,
            ( SELECT string_agg(((l.jobname || ' ('::text) || l.schedule) || ')'::text, ', '::text ORDER BY l.jobname) AS string_agg
                   FROM live l
                     LEFT JOIN declared d USING (jobname)
                  WHERE d.jobname IS NULL) AS unregistered,
            ( SELECT string_agg(((d.jobname || ' (declared '::text) || d.schedule) || ')'::text, ', '::text ORDER BY d.jobname) AS string_agg
                   FROM declared d
                     LEFT JOIN live l USING (jobname)
                  WHERE l.jobname IS NULL) AS missing,
            ( SELECT string_agg(l.jobname, ', '::text ORDER BY l.jobname) AS string_agg
                   FROM declared d
                     JOIN live l USING (jobname)
            ...
            ( SELECT count(*) AS count
                   FROM declared) AS declared_count,
            ( SELECT count(*) AS count
                   FROM live) AS live_count
        )
 SELECT 'MON-025'::text AS check_code,
        CASE
            WHEN declared_count = 0 OR live_count = 0 THEN 'UNKNOWN'::text
            WHEN mismatched IS NOT NULL OR unregistered IS NOT NULL OR missing IS NOT NULL OR off_undeclared IS NOT NULL OR on_declared_off IS NOT NULL THEN 'PROBLEM'::text
            ELSE 'OK'::text
        END AS state,
        CASE
            WHEN declared_count = 0 THEN 'Nothing to evaluate: cron_schedule_registry is empty, so there is nothing to '::text || 'compare the live jobs against. This is not a pass.'::text
            WHEN live_count = 0 THEN 'Nothing to evaluate: cron.job holds no jobs at all. Either pg_cron is not '::text || 'installed here or every scheduled job is gone. This is not a pass.'::text
            ELSE TRIM(BOTH ' '::text FROM (((((COALESCE(('Schedule differs from the migration that declares it: '::text || mismatched) || '. '::text, ''::text) || COALESCE(('Scheduled but declared nowhere, so nothing governs it: '::text || unregistered) || '. '::text, ''::text)) || COALESCE(('Declared but not scheduled: '::text || missing) || '. '::text, ''::text)) || COALESCE(('Switched off, but declared on: '::text || off_undeclared) || '. '::text, ''::text)) || COALESCE(('Running, but declared off: '::text || on_declared_off) || '. '::text, ''::text)) ||
            CASE
                WHEN mismatched IS NULL AND unregistered IS NULL AND missing IS NULL AND off_undeclared IS NULL AND on_declared_off IS NULL THEN (((('All '::text || declared_count) || ' declared job(s) are scheduled, on the schedule '::text) || 'their migration declares, and on or off as declared. '::text) || live_count) || ' job(s) live. '::text
                ELSE ''::text
            END) || COALESCE(('Switched off, as declared: '::text || off_as_declared) || '.'::text, ''::text))
        END AS detail,
    now() AS last_run
   FROM findings f;
...
CREATE OR REPLACE VIEW public.mon_026 AS
 WITH snap AS (
         SELECT suppression_reconciliation_snapshot.id,
            suppression_reconciliation_snapshot.uploaded_count,
            suppression_reconciliation_snapshot.blocked_count,
            suppression_reconciliation_snapshot.checked_count,
            suppression_reconciliation_snapshot.unreconciled_count,
            suppression_reconciliation_snapshot.unreachable_count,
            suppression_reconciliation_snapshot.settling_count,
            suppression_reconciliation_snapshot.invariant_breach_count,
            suppression_reconciliation_snapshot.incomplete,
            suppression_reconciliation_snapshot.unreconciled_prospect_ids,
            suppression_reconciliation_snapshot.detail,
            suppression_reconciliation_snapshot.computed_at,
            suppression_reconciliation_snapshot.uncarried_count,
            suppression_reconciliation_snapshot.carry_failed_count
           FROM suppression_reconciliation_snapshot
          WHERE suppression_reconciliation_snapshot.id = 1
        )
 SELECT 'MON-026'::text AS check_code,
        CASE
            WHEN NOT (EXISTS ( SELECT 1
               FROM snap)) THEN 'UNKNOWN'::text
            WHEN (( SELECT snap.computed_at
               FROM snap)) < (now() - '01:30:00'::interval) THEN 'PROBLEM'::text
            WHEN ( SELECT snap.incomplete
               FROM snap) THEN 'PROBLEM'::text
            WHEN (( SELECT snap.carry_failed_count
               FROM snap)) > 0 THEN 'PROBLEM'::text
            WHEN (( SELECT snap.uncarried_count
               FROM snap)) > 0 THEN 'PROBLEM'::text
            WHEN (( SELECT snap.uploaded_count
               FROM snap)) = 0 THEN 'UNKNOWN'::text
            WHEN (( SELECT snap.unreconciled_count
               FROM snap)) > 0 THEN 'PROBLEM'::text
            WHEN (( SELECT snap.invariant_breach_count
               FROM snap)) > 0 THEN 'PROBLEM'::text
            WHEN (( SELECT snap.unreachable_count
               FROM snap)) > 0 THEN 'PROBLEM'::text
            ELSE 'OK'::text
        END AS state,
        CASE
            WHEN NOT (EXISTS ( SELECT 1
               FROM snap)) THEN 'No suppression reconciliation has run yet. Expected until the first suppression-reconcile firing after deploy; if it persists, the sweep is not running and nothing is comparing our suppression list against the provider.'::text
            WHEN (( SELECT snap.computed_at
               FROM snap)) < (now() - '01:30:00'::interval) THEN ((('The suppression reconciliation is '::text || round(EXTRACT(epoch FROM now() - (( SELECT snap.computed_at
               FROM snap))) / 60::numeric)::text) || ' minutes old, past the 90-minute limit. It last said: "'::text) || (( SELECT snap.detail
               FROM snap))) || '" That answer describes a window that has moved on, so it is not being reported as current. suppression-reconcile runs every 30 minutes: check it is still running.'::text
            WHEN ( SELECT snap.incomplete
               FROM snap) THEN ( SELECT snap.detail
               FROM snap)
            WHEN (( SELECT snap.carry_failed_count
               FROM snap)) > 0 THEN ( SELECT snap.detail
               FROM snap)
            WHEN (( SELECT snap.uncarried_count
               FROM snap)) > 0 THEN ( SELECT snap.detail
               FROM snap)
            WHEN (( SELECT snap.uploaded_count
               FROM snap)) = 0 THEN 'Nothing to evaluate against the provider: no prospect has been uploaded, so there is nothing it could still be sending to. Every active suppression has reached the provider, which was checked separately. This is not a pass.'::text
            ELSE ( SELECT snap.detail
               FROM snap)
        END AS detail,
    ( SELECT snap.computed_at
...
CREATE OR REPLACE VIEW public.mon_027 AS
 WITH cursors AS (
         SELECT polling_cursors.resource,
            polling_cursors.last_run_at,
            polling_cursors.last_cursor,
            polling_cursors.error_count,
            polling_cursors.last_error,
            polling_cursors.updated_at
           FROM polling_cursors
          WHERE polling_cursors.source = 'instantly'::text
        ), failing AS (
         SELECT count(*) AS n,
            min(cursors.resource) AS first_resource,
            min(cursors.last_error) AS first_error,
            sum(cursors.error_count) AS total_errors
           FROM cursors
          WHERE cursors.error_count > 0
        ), stalled AS (
         SELECT count(*) AS n,
            min(cursors.resource) AS first_resource
           FROM cursors
          WHERE cursors.resource = 'replies'::text AND cursors.last_error IS NOT NULL AND cursors.last_run_at > (now() - '02:00:00'::interval)
        )
 SELECT 'MON-027'::text AS check_code,
        CASE
            WHEN (( SELECT count(*) AS count
               FROM cursors)) = 0 THEN 'UNKNOWN'::text
            WHEN (( SELECT stalled.n
               FROM stalled)) > 0 THEN 'PROBLEM'::text
            WHEN (( SELECT failing.n
               FROM failing)) > 0 THEN 'PROBLEM'::text
            ELSE 'OK'::text
        END AS state,
        CASE
            WHEN (( SELECT count(*) AS count
               FROM cursors)) = 0 THEN 'No polling cursor rows exist, so there is nothing to evaluate. This is not a pass: it means the poller has never written its state for this source.'::text
            WHEN (( SELECT stalled.n
               FROM stalled)) > 0 THEN ('The reply cursor is being HELD and is not advancing. Last error: '::text || COALESCE(( SELECT cursors.last_error
               FROM cursors
              WHERE cursors.resource = 'replies'::text), 'none recorded'::text)) || '. Replies after the held page are not being read until this clears. This is the deliberate stall that replaces silent reply loss: the page is re-fetched every run and rows that already landed dedupe, so no data is lost while it is red.'::text
            WHEN (( SELECT failing.n
               FROM failing)) > 0 THEN ((((((((( SELECT failing.n
               FROM failing))::text) || ' polling resource(s) carrying errors, '::text) || ((( SELECT failing.total_errors
               FROM failing))::text)) || ' failure(s) total since the last clean run. '::text) || 'First: '::text) || COALESCE(( SELECT failing.first_resource
               FROM failing), '?'::text)) || ' - '::text) || COALESCE(( SELECT failing.first_error
               FROM failing), 'no detail'::text)
            ELSE ('All '::text || ((( SELECT count(*) AS count
               FROM cursors))::text)) || ' polling cursor(s) clean, no recorded errors.'::text
        END AS detail,
    ( SELECT max(cursors.last_run_at) AS max
           FROM cursors) AS last_run;

CREATE OR REPLACE VIEW public.mon_028 AS
 WITH default_threshold AS (
         SELECT reply_draft_ageing_config.threshold_hours
           FROM reply_draft_ageing_config
          WHERE reply_draft_ageing_config.organisation_id IS NULL
        ), ageing AS (
         SELECT d.id,
            d.status,
            d.created_at,
            COALESCE(ovr.threshold_hours, ( SELECT default_threshold.threshold_hours
                   FROM default_threshold)) AS threshold_hours,
            round(EXTRACT(epoch FROM now() - d.created_at) / 3600.0, 1) AS age_hours
           FROM reply_drafts d
             JOIN organisations o ON o.id = d.organisation_id
             LEFT JOIN reply_draft_ageing_config ovr ON ovr.organisation_id = d.organisation_id
          WHERE o.archived_at IS NULL AND (d.status = ANY (ARRAY['pending'::text, 'manual_required'::text, 'draft_failed'::text, 'send_failed'::text]))
        ), overdue AS (
         SELECT count(*) AS n,
            max(ageing.age_hours) AS oldest_hours,
            min(ageing.status) AS a_status
           FROM ageing
          WHERE ageing.threshold_hours IS NOT NULL AND ageing.age_hours > ageing.threshold_hours::numeric
        )
 SELECT 'MON-028'::text AS check_code,
        CASE
            WHEN NOT (EXISTS ( SELECT 1
               FROM default_threshold)) THEN 'UNKNOWN'::text
            WHEN (( SELECT overdue.n
               FROM overdue)) > 0 THEN 'PROBLEM'::text
            ELSE 'OK'::text
        END AS state,
        CASE
            WHEN NOT (EXISTS ( SELECT 1
               FROM default_threshold)) THEN 'No default row in reply_draft_ageing_config, so no draft can be compared against anything and every draft would pass. This is not a pass: restore the default row.'::text
            WHEN (( SELECT overdue.n
               FROM overdue)) > 0 THEN ((((((( SELECT overdue.n
               FROM overdue))::text) || ' reply draft(s) waiting on an operator past their threshold. Oldest: '::text) || ((( SELECT overdue.oldest_hours
               FROM overdue))::text)) || ' hours, status '::text) || COALESCE(( SELECT overdue.a_status
               FROM overdue), '?'::text)) || '. A prospect replied and nobody has answered.'::text
            ELSE ((('No reply draft is overdue. '::text || ((( SELECT count(*) AS count
               FROM ageing))::text)) || ' draft(s) currently waiting, threshold '::text) || ((( SELECT default_threshold.threshold_hours
               FROM default_threshold))::text)) || 'h by default.'::text
        END AS detail,
    ( SELECT min(ageing.created_at) AS min
           FROM ageing) AS last_run;

CREATE OR REPLACE VIEW public.mon_029 AS
 WITH snap AS (
         SELECT reply_reconciliation_snapshot.id,
            reply_reconciliation_snapshot.campaigns_checked,
            reply_reconciliation_snapshot.provider_reply_count,
            reply_reconciliation_snapshot.stored_reply_count,
            reply_reconciliation_snapshot.missing_count,
            reply_reconciliation_snapshot.unreachable_campaigns,
            reply_reconciliation_snapshot.incomplete,
            reply_reconciliation_snapshot.missing_sample,
            reply_reconciliation_snapshot.detail,
            reply_reconciliation_snapshot.computed_at
           FROM reply_reconciliation_snapshot
          WHERE reply_reconciliation_snapshot.id = 1
        )
 SELECT 'MON-029'::text AS check_code,
        CASE
            WHEN NOT (EXISTS ( SELECT 1
               FROM snap)) THEN 'UNKNOWN'::text
            WHEN (( SELECT snap.computed_at
               FROM snap)) < (now() - '01:30:00'::interval) THEN 'PROBLEM'::text
            WHEN (( SELECT snap.unreachable_campaigns
               FROM snap)) > 0 THEN 'PROBLEM'::text
            WHEN (( SELECT snap.missing_count
               FROM snap)) > 0 THEN 'PROBLEM'::text
            WHEN ( SELECT snap.incomplete
               FROM snap) THEN 'PROBLEM'::text
            WHEN (( SELECT snap.provider_reply_count
               FROM snap)) = 0 THEN 'UNKNOWN'::text
            ELSE 'OK'::text
        END AS state,
        CASE
            WHEN NOT (EXISTS ( SELECT 1
               FROM snap)) THEN 'No reply reconciliation has run yet. Expected until the first instantly-poll run after deploy; if it persists, the sweep is not running.'::text
            WHEN (( SELECT snap.computed_at
               FROM snap)) < (now() - '01:30:00'::interval) THEN ((('The reply reconciliation verdict is '::text || round(EXTRACT(epoch FROM now() - (( SELECT snap.computed_at
               FROM snap))) / 60::numeric)::text) || ' minutes old, past the 90-minute limit. It last said: '::text) || COALESCE(( SELECT snap.detail
               FROM snap), 'no detail'::text)) || ' That answer describes a window that has moved on. instantly-poll writes this every 15 minutes: check it is still running.'::text
            ELSE COALESCE(( SELECT snap.detail
               FROM snap), 'no detail'::text)
        END AS detail,
    ( SELECT snap.computed_at
           FROM snap) AS last_run;
...
CREATE OR REPLACE VIEW public.mon_030 AS
 WITH outstanding AS (
         SELECT count(*) AS n,
            count(*) FILTER (WHERE email_delivery_failures.audience = 'operator'::text) AS n_operator,
            count(*) FILTER (WHERE email_delivery_failures.stage = 'content_validation'::text) AS n_validation,
            max(email_delivery_failures.created_at) AS newest,
            min(email_delivery_failures.created_at) AS oldest,
            (array_agg(email_delivery_failures.subject ORDER BY email_delivery_failures.created_at DESC))[1] AS newest_subject,
            (array_agg(email_delivery_failures.error_message ORDER BY email_delivery_failures.created_at DESC))[1] AS newest_error
           FROM email_delivery_failures
          WHERE email_delivery_failures.resolved_at IS NULL
        )
 SELECT 'MON-030'::text AS check_code,
        CASE
            WHEN (( SELECT outstanding.n
               FROM outstanding)) > 0 THEN 'PROBLEM'::text
            ELSE 'OK'::text
        END AS state,
        CASE
            WHEN (( SELECT outstanding.n
               FROM outstanding)) > 0 THEN ((((((((((( SELECT outstanding.n
               FROM outstanding))::text) || ' email(s) did not send and are unresolved, '::text) || ((( SELECT outstanding.n_operator
               FROM outstanding))::text)) || ' of them operator alerts. '::text) || ((( SELECT outstanding.n_validation
               FROM outstanding))::text)) || ' rejected before reaching the provider. '::text) || 'Most recent: "'::text) || COALESCE(( SELECT outstanding.newest_subject
               FROM outstanding), '?'::text)) || '" -> '::text) || COALESCE(( SELECT outstanding.newest_error
               FROM outstanding), '?'::text)
            ELSE 'No unresolved email delivery failures.'::text
        END AS detail,
    ( SELECT outstanding.newest
           FROM outstanding) AS last_run;

CREATE OR REPLACE VIEW public.mon_031 AS
 WITH quarantined AS (
         SELECT count(*) FILTER (WHERE unattributed_replies.resolved_at IS NULL) AS unresolved,
            count(*) FILTER (WHERE unattributed_replies.resolved_at IS NULL AND unattributed_replies.body_redacted_at IS NOT NULL) AS redacted,
            min(COALESCE(unattributed_replies.reply_written_at, unattributed_replies.first_seen_at)) FILTER (WHERE unattributed_replies.resolved_at IS NULL) AS oldest,
            count(*) FILTER (WHERE unattributed_replies.resolved_at IS NULL AND unattributed_replies.reply_written_at IS NULL) AS no_write_time,
            (array_agg(DISTINCT unattributed_replies.provider_campaign_id) FILTER (WHERE unattributed_replies.resolved_at IS NULL))[1:3] AS campaigns
           FROM unattributed_replies
        )
 SELECT 'MON-031'::text AS check_code,
        CASE
            WHEN (( SELECT quarantined.unresolved
               FROM quarantined)) > 0 THEN 'PROBLEM'::text
            ELSE 'OK'::text
        END AS state,
        CASE
            WHEN (( SELECT quarantined.unresolved
               FROM quarantined)) > 0 THEN (((((((((( SELECT quarantined.unresolved
               FROM quarantined))::text) || ' repl(y/ies) arrived that we could not attribute to any client. Nothing has '::text) || 'been sent back to the people who wrote them. Provider campaign(s): '::text) || COALESCE(array_to_string(( SELECT quarantined.campaigns
               FROM quarantined), ', '::text), '(none recorded)'::text)) || '. Longest anyone has been waiting: '::text) || COALESCE(round(EXTRACT(epoch FROM now() - (( SELECT quarantined.oldest
               FROM quarantined))) / 3600.0, 1)::text, '?'::text)) || ' hours since they wrote.'::text) ||
            CASE
                WHEN (( SELECT quarantined.no_write_time
                   FROM quarantined)) > 0 THEN ((' '::text || ((( SELECT quarantined.no_write_time
                   FROM quarantined))::text)) || ' of these carried no send time, so their wait is measured from when we '::text) || 'first held them and is a floor, not the real figure.'::text
                ELSE ''::text
            END) ||
            CASE
                WHEN (( SELECT quarantined.redacted
                   FROM quarantined)) > 0 THEN ((' '::text || ((( SELECT quarantined.redacted
                   FROM quarantined))::text)) || ' are past 30 days and have had their contents redacted, '::text) || 'so they can no longer be replayed.'::text
                ELSE ''::text
            END
            ELSE ('No unattributed replies are waiting. This is not proof the quarantine write '::text || 'ran: an empty table and a broken sweep read identically here. MON-002 and '::text) || 'MON-027 are what report whether the poller is running at all.'::text
        END AS detail,
    ( SELECT quarantined.oldest
           FROM quarantined) AS last_run;

CREATE OR REPLACE VIEW public.mon_032 AS
 WITH recent AS (
         SELECT count(*) AS total,
            count(*) FILTER (WHERE dashboard_failures.kind = 'render'::text) AS renders,
            count(*) FILTER (WHERE dashboard_failures.kind = 'read'::text) AS reads,
            count(DISTINCT dashboard_failures.organisation_id) AS orgs,
            max(dashboard_failures.occurred_at) AS latest,
            (array_agg(DISTINCT dashboard_failures.source))[1:3] AS sources
           FROM dashboard_failures
          WHERE dashboard_failures.occurred_at > (now() - '01:00:00'::interval)
        )
 SELECT 'MON-032'::text AS check_code,
        CASE
            WHEN (( SELECT recent.renders
               FROM recent)) > 0 THEN 'PROBLEM'::text
            WHEN (( SELECT recent.reads
               FROM recent)) > 0 THEN 'PROBLEM'::text
            ELSE 'OK'::text
        END AS state,
        CASE
            WHEN (( SELECT recent.total
               FROM recent)) > 0 THEN ((((((((((((( SELECT recent.total
               FROM recent))::text) || ' dashboard failure(s) in the last hour, affecting '::text) || ((( SELECT recent.orgs
               FROM recent))::text)) || ' organisation(s). '::text) || ((( SELECT recent.renders
               FROM recent))::text)) || ' were whole-page render failures, where the client saw a recovery card '::text) || 'instead of their dashboard. '::text) || ((( SELECT recent.reads
               FROM recent))::text)) || ' were single reads that failed, were refused, or timed out, where the page '::text) || 'rendered with a degraded card. Source(s): '::text) || COALESCE(array_to_string(( SELECT recent.sources
               FROM recent), ', '::text), '(none)'::text)) || '.'::text
            ELSE (((('No dashboard read or render failure in the last hour. THIS IS NOT PROOF THE '::text || 'DASHBOARD IS HEALTHY. It proves only that the recorder wrote nothing: an hour '::text) || 'with no client logged in reads exactly the same as an hour with no failures, '::text) || 'and this check cannot tell them apart because nothing here observes a page. It '::text) || 'watches one product failure that learned how to write a row. Every other check '::text) || 'on this board watches the database.'::text
        END AS detail,
    ( SELECT recent.latest
           FROM recent) AS last_run;
...
CREATE TRIGGER strategy_documents_set_updated_at BEFORE UPDATE ON public.strategy_documents FOR EACH ROW EXECUTE FUNCTION set_updated_at();
DO $baseline$
BEGIN
...
CREATE POLICY clients_read_own_strategy_docs ON public.strategy_documents AS PERMISSIVE FOR SELECT TO authenticated
  USING (((organisation_id = get_my_organisation_id()) AND (status = ANY (ARRAY['active'::text, 'approved'::text, 'archived'::text]))));
...
GRANT DELETE ON public.cron_schedule_registry TO service_role;
GRANT INSERT ON public.cron_schedule_registry TO service_role;
GRANT REFERENCES ON public.cron_schedule_registry TO service_role;
GRANT SELECT ON public.cron_schedule_registry TO service_role;
GRANT TRIGGER ON public.cron_schedule_registry TO service_role;
GRANT TRUNCATE ON public.cron_schedule_registry TO service_role;
GRANT UPDATE ON public.cron_schedule_registry TO service_role;
GRANT DELETE ON public.dashboard_failures TO service_role;
GRANT INSERT ON public.dashboard_failures TO service_role;
GRANT REFERENCES ON public.dashboard_failures TO service_role;
GRANT SELECT ON public.dashboard_failures TO service_role;
GRANT TRIGGER ON public.dashboard_failures TO service_role;
GRANT TRUNCATE ON public.dashboard_failures TO service_role;
GRANT UPDATE ON public.dashboard_failures TO service_role;
...
GRANT DELETE ON public.email_delivery_failures TO service_role;
GRANT INSERT ON public.email_delivery_failures TO service_role;
GRANT REFERENCES ON public.email_delivery_failures TO service_role;
GRANT SELECT ON public.email_delivery_failures TO service_role;
GRANT TRIGGER ON public.email_delivery_failures TO service_role;
GRANT TRUNCATE ON public.email_delivery_failures TO service_role;
GRANT UPDATE ON public.email_delivery_failures TO service_role;
...
GRANT DELETE ON public.gateway_retry_counts TO service_role;
GRANT INSERT ON public.gateway_retry_counts TO service_role;
GRANT REFERENCES ON public.gateway_retry_counts TO service_role;
GRANT SELECT ON public.gateway_retry_counts TO service_role;
GRANT TRIGGER ON public.gateway_retry_counts TO service_role;
GRANT TRUNCATE ON public.gateway_retry_counts TO service_role;
GRANT UPDATE ON public.gateway_retry_counts TO service_role;
...
GRANT DELETE ON public.mon_025 TO service_role;
GRANT INSERT ON public.mon_025 TO service_role;
GRANT REFERENCES ON public.mon_025 TO service_role;
GRANT SELECT ON public.mon_025 TO service_role;
GRANT TRIGGER ON public.mon_025 TO service_role;
GRANT TRUNCATE ON public.mon_025 TO service_role;
GRANT UPDATE ON public.mon_025 TO service_role;
GRANT DELETE ON public.mon_026 TO service_role;
GRANT INSERT ON public.mon_026 TO service_role;
GRANT REFERENCES ON public.mon_026 TO service_role;
GRANT SELECT ON public.mon_026 TO service_role;
GRANT TRIGGER ON public.mon_026 TO service_role;
GRANT TRUNCATE ON public.mon_026 TO service_role;
GRANT UPDATE ON public.mon_026 TO service_role;
GRANT DELETE ON public.mon_027 TO service_role;
GRANT INSERT ON public.mon_027 TO service_role;
GRANT REFERENCES ON public.mon_027 TO service_role;
GRANT SELECT ON public.mon_027 TO service_role;
GRANT TRIGGER ON public.mon_027 TO service_role;
GRANT TRUNCATE ON public.mon_027 TO service_role;
GRANT UPDATE ON public.mon_027 TO service_role;
GRANT DELETE ON public.mon_028 TO service_role;
GRANT INSERT ON public.mon_028 TO service_role;
GRANT REFERENCES ON public.mon_028 TO service_role;
GRANT SELECT ON public.mon_028 TO service_role;
GRANT TRIGGER ON public.mon_028 TO service_role;
GRANT TRUNCATE ON public.mon_028 TO service_role;
GRANT UPDATE ON public.mon_028 TO service_role;
GRANT DELETE ON public.mon_029 TO service_role;
GRANT INSERT ON public.mon_029 TO service_role;
GRANT REFERENCES ON public.mon_029 TO service_role;
GRANT SELECT ON public.mon_029 TO service_role;
GRANT TRIGGER ON public.mon_029 TO service_role;
GRANT TRUNCATE ON public.mon_029 TO service_role;
GRANT UPDATE ON public.mon_029 TO service_role;
GRANT DELETE ON public.mon_030 TO service_role;
GRANT INSERT ON public.mon_030 TO service_role;
GRANT REFERENCES ON public.mon_030 TO service_role;
GRANT SELECT ON public.mon_030 TO service_role;
GRANT TRIGGER ON public.mon_030 TO service_role;
GRANT TRUNCATE ON public.mon_030 TO service_role;
GRANT UPDATE ON public.mon_030 TO service_role;
GRANT DELETE ON public.mon_031 TO service_role;
GRANT INSERT ON public.mon_031 TO service_role;
GRANT REFERENCES ON public.mon_031 TO service_role;
GRANT SELECT ON public.mon_031 TO service_role;
GRANT TRIGGER ON public.mon_031 TO service_role;
GRANT TRUNCATE ON public.mon_031 TO service_role;
GRANT UPDATE ON public.mon_031 TO service_role;
GRANT DELETE ON public.mon_032 TO service_role;
GRANT INSERT ON public.mon_032 TO service_role;
GRANT REFERENCES ON public.mon_032 TO service_role;
GRANT SELECT ON public.mon_032 TO service_role;
GRANT TRIGGER ON public.mon_032 TO service_role;
GRANT TRUNCATE ON public.mon_032 TO service_role;
GRANT UPDATE ON public.mon_032 TO service_role;
...
GRANT DELETE ON public.reply_draft_ageing_config TO service_role;
GRANT INSERT ON public.reply_draft_ageing_config TO service_role;
GRANT REFERENCES ON public.reply_draft_ageing_config TO service_role;
GRANT SELECT ON public.reply_draft_ageing_config TO service_role;
GRANT TRIGGER ON public.reply_draft_ageing_config TO service_role;
GRANT TRUNCATE ON public.reply_draft_ageing_config TO service_role;
GRANT UPDATE ON public.reply_draft_ageing_config TO service_role;
...
GRANT DELETE ON public.reply_reconciliation_snapshot TO service_role;
GRANT INSERT ON public.reply_reconciliation_snapshot TO service_role;
GRANT REFERENCES ON public.reply_reconciliation_snapshot TO service_role;
GRANT SELECT ON public.reply_reconciliation_snapshot TO service_role;
GRANT TRIGGER ON public.reply_reconciliation_snapshot TO service_role;
GRANT TRUNCATE ON public.reply_reconciliation_snapshot TO service_role;
GRANT UPDATE ON public.reply_reconciliation_snapshot TO service_role;
...
GRANT DELETE ON public.sourcing_runs TO service_role;
GRANT INSERT ON public.sourcing_runs TO service_role;
GRANT REFERENCES ON public.sourcing_runs TO service_role;
GRANT SELECT ON public.sourcing_runs TO service_role;
GRANT TRIGGER ON public.sourcing_runs TO service_role;
GRANT TRUNCATE ON public.sourcing_runs TO service_role;
GRANT UPDATE ON public.sourcing_runs TO service_role;
...
GRANT DELETE ON public.suppression_reconciliation_snapshot TO service_role;
GRANT INSERT ON public.suppression_reconciliation_snapshot TO service_role;
GRANT REFERENCES ON public.suppression_reconciliation_snapshot TO service_role;
GRANT SELECT ON public.suppression_reconciliation_snapshot TO service_role;
GRANT TRIGGER ON public.suppression_reconciliation_snapshot TO service_role;
GRANT TRUNCATE ON public.suppression_reconciliation_snapshot TO service_role;
GRANT UPDATE ON public.suppression_reconciliation_snapshot TO service_role;
...
GRANT DELETE ON public.tuning_rounds TO service_role;
GRANT INSERT ON public.tuning_rounds TO service_role;
GRANT REFERENCES ON public.tuning_rounds TO service_role;
GRANT SELECT ON public.tuning_rounds TO service_role;
GRANT TRIGGER ON public.tuning_rounds TO service_role;
GRANT TRUNCATE ON public.tuning_rounds TO service_role;
GRANT UPDATE ON public.tuning_rounds TO service_role;
GRANT DELETE ON public.tuning_runs TO service_role;
GRANT INSERT ON public.tuning_runs TO service_role;
GRANT REFERENCES ON public.tuning_runs TO service_role;
GRANT SELECT ON public.tuning_runs TO service_role;
GRANT TRIGGER ON public.tuning_runs TO service_role;
GRANT TRUNCATE ON public.tuning_runs TO service_role;
GRANT UPDATE ON public.tuning_runs TO service_role;
GRANT DELETE ON public.unattributed_bookings TO service_role;
GRANT INSERT ON public.unattributed_bookings TO service_role;
GRANT REFERENCES ON public.unattributed_bookings TO service_role;
GRANT SELECT ON public.unattributed_bookings TO service_role;
GRANT TRIGGER ON public.unattributed_bookings TO service_role;
GRANT TRUNCATE ON public.unattributed_bookings TO service_role;
GRANT UPDATE ON public.unattributed_bookings TO service_role;
GRANT DELETE ON public.unattributed_replies TO service_role;
GRANT INSERT ON public.unattributed_replies TO service_role;
GRANT REFERENCES ON public.unattributed_replies TO service_role;
GRANT SELECT ON public.unattributed_replies TO service_role;
GRANT TRIGGER ON public.unattributed_replies TO service_role;
GRANT TRUNCATE ON public.unattributed_replies TO service_role;
GRANT UPDATE ON public.unattributed_replies TO service_role;
...
GRANT EXECUTE ON FUNCTION public.approve_document_suggestion(p_suggestion_id uuid, p_reviewer_id uuid, p_plain_text text) TO service_role;
...
GRANT EXECUTE ON FUNCTION public.promote_strategy_doc_version(p_org_id uuid, p_doc_type text, p_segment_id uuid, p_content jsonb, p_update_trigger text, p_revision_note text, p_change_summary text, p_plain_text text, p_generated_by_model text) TO service_role;
...
GRANT EXECUTE ON FUNCTION public.record_gateway_retry(p_method text) TO service_role;
GRANT EXECUTE ON FUNCTION public.record_job_spend(p_job_id uuid, p_detail jsonb) TO service_role;
GRANT EXECUTE ON FUNCTION public.revert_strategy_doc_version(p_document_id uuid) TO service_role;
...
-- ── COMMENTS ──────────────────────────────────────────────────────────
COMMENT ON COLUMN public.campaigns.contacted_count IS 'People with at least one send, NOT emails. Sourced from the outbound provider''s new_leads_contacted_count and bounded by its leads_count. Do NOT source this from a field merely named contacted_count: on Instantly that field read 52 against 24 leads on 2026-09-03, having matched the people count exactly on 2026-08-24, so it is not reliably people and is not reliably emails either. sent_count is the email counter. This is the number behind "prospects contacted" on the client overview.';
...
COMMENT ON COLUMN public.cron_schedule_registry.active IS 'Whether the migrations declare this job ON. False means switched off on purpose, and MON-025 and MON-001 read it. Held to the migration files by cron-schedule-registry.test.ts.';
COMMENT ON COLUMN public.document_suggestions.generated_by_model IS 'The Anthropic model id that produced suggested_value. Carried onto strategy_documents when the suggestion is approved, so a promoted document keeps the model that wrote it rather than the model current at approval time.';
...
COMMENT ON COLUMN public.intake_website_pages.extraction_truncated IS 'True when the raw page extraction exceeded MAX_CHARS_PER_PAGE in fetch-website.ts and was cut before storage. Set at fetch time. Rows predating this column were backfilled by length, since truncation is applied after trim and therefore lands on exactly the cap.';
COMMENT ON COLUMN public.meetings.bill_unconfirmed_after IS 'The last instant of the month after the meeting month, UTC. If still unconfirmed after this, the meeting bills unconfirmed (2026-08-24 decision). A calendar date, never a fixed number of days from the meeting.';
COMMENT ON COLUMN public.meetings.billable_basis IS 'How the meeting became billable: client_confirmed, operator_marked, or unconfirmed_backstop. Required whenever is_billable is true, and shown on every billing view.';
COMMENT ON COLUMN public.monitor_events.alert_pending IS 'True on a PROBLEM row whose alert has not been sent yet. The monitor sweep sends it on the second consecutive PROBLEM reading and clears this; a recovery first clears it unsent.';
COMMENT ON COLUMN public.organisations.booking_url IS 'The booking link sent to prospects in reply emails. Tool-agnostic: any booking tool''s link works for SENDING. Only booking DETECTION depends on which tool it is.';
COMMENT ON COLUMN public.organisations.currency IS 'The currency every money figure on this client''s dashboard renders in. USD is the decided default (2026-09-15). GBP and EUR remain valid so an existing client is never silently re-denominated. Read by src/lib/currency/format-currency.ts; never assume a symbol at a render site.';
COMMENT ON COLUMN public.organisations.sourcing_revenue_filter_enabled IS 'Per-client opt-in for the ICP revenue band as a sourcing filter. Default false. Read when the spec is derived at ICP approval, so a change takes effect at the next approval. The provider filter excludes every company with no revenue figure recorded.';
...
COMMENT ON COLUMN public.prospects.outbound_suppression_at IS 'When outbound_suppression_status was last written. Not the suppression time: suppressed_at is that. These two differing is the interval during which our record and the provider disagreed.';
COMMENT ON COLUMN public.prospects.outbound_suppression_error IS 'Why the provider was not told, when status is failed. Constrained to failed rows only.';
COMMENT ON COLUMN public.prospects.outbound_suppression_status IS 'Whether the sending provider was actually told to stop, for a prospect this system has suppressed. not_required = no provider lead exists. confirmed = called and READ BACK. failed = the call failed, the read-back disagreed, or no lead could be resolved. NULL = never attempted, which on a suppressed row means something bypassed the shared path.';
...
COMMENT ON COLUMN public.prospects.personalisation_subject IS 'Written Email 1 subject replacing the variant authored subject at composition. NULL keeps the authored subject. Written only when the personalised version wins the judge AND the generated subject passed its soft gate, so it may be NULL while personalisation_trigger is set.';
COMMENT ON COLUMN public.prospects.send_hold_at IS 'When an operator placed a durable hold on sending to this prospect. Non-null means held. Read by resolveSendEligibility BEFORE the country rule and before every verification verdict, so re-verification cannot clear it. Never set by an automated path.';
COMMENT ON COLUMN public.prospects.send_hold_by IS 'The operator who placed the hold. NULL means unattributed: the hold is real but the actor was not recorded, which is the case for holds applied by direct UPDATE before this column existed. NULL is never inferred or backfilled with a guess.';
COMMENT ON COLUMN public.prospects.send_hold_reason IS 'Free text: why this prospect is held. Required in practice for any new hold. This is the column that lets a future reader tell a RULE from a HAND EDIT, which is the defect that motivated the change: a row held with no reason is indistinguishable from a bug.';
COMMENT ON COLUMN public.reply_handling_actions.link_sent_at IS 'When a booking link was actually sent to the prospect, written once at send time and only on provider success. NULL means no link went out. Do not measure send time from updated_at: any later write to the row moves it and makes time-to-booking look faster than it was.';
...
COMMENT ON COLUMN public.strategy_documents.generated_by_model IS 'The Anthropic model id that produced this document''s content, e.g. claude-opus-4-6. NULL on the 69 rows that predate 2026-09-08, and that NULL is honest: it is not recoverable, because nothing recorded it. Never inferred from a date.';
COMMENT ON COLUMN public.strategy_documents.icp_filter_spec_refusal IS 'Why this version''s icp_filter_spec could not be built: {reason, detail, recorded_at}. Written by persistIcpFilterSpec, cleared when a spec is stored. NULL = no refusal recorded.';
COMMENT ON COLUMN public.strategy_documents.stale_reason IS 'Why is_stale was set. NULL means an upstream document changed (promote_strategy_doc_version, which infers its own wording). A non-null value names a different cause, currently only intake_answer_changed:<field_key>. Never shown raw to a client; stale-documents.ts maps it to client-facing wording.';
COMMENT ON COLUMN public.suppressed_emails.carry_attempted_at IS 'When the carry was LAST ATTEMPTED, set on failure as well as success. Not a success stamp: success is carry_status. Used for retry backoff.';
COMMENT ON COLUMN public.suppressed_emails.carry_status IS 'Whether this suppression reached the sending provider. NULL = never attempted. not_required = the provider holds no lead for this address. confirmed = every lead it held was stopped and read back. failed = see carry_error. Written ONLY by carryOneSuppression in src/lib/suppression/carry.ts.';
COMMENT ON COLUMN public.suppression_reconciliation_snapshot.carry_failed_count IS 'Active suppressions whose last carry attempt to the provider failed. Retried by the carry sweep after its backoff; red here until it succeeds.';
COMMENT ON COLUMN public.suppression_reconciliation_snapshot.uncarried_count IS 'Active suppressions never carried to the sending provider, past the grace window. Read from suppressed_emails directly, so it covers addresses with no uploaded prospect row, which the prospect-based pass cannot see at all.';
COMMENT ON COLUMN public.tuning_runs.fit_of_resolved IS 'Share of the resolved sample judged a fit. Compare runs on this, never on fit of all sampled, which moves with the research success rate too.';
COMMENT ON COLUMN public.tuning_runs.rubric_source IS 'stored_conditions: graded against the client''s stored fit dimensions, stable between runs. derived_this_run: graded against a rubric derived by a model call in that run, which is not comparable across runs.';
COMMENT ON COLUMN public.unattributed_replies.reply_written_at IS 'When the prospect actually wrote, taken from the provider payload at quarantine time. NULL when the payload carried no usable timestamp. Survives the 30-day redaction, which is the whole reason it is a column and not an expression over raw_data.';
COMMENT ON TABLE public.dashboard_failures IS 'One row per dashboard read failure or render failure. Written by recordDashboardFailure. Read by mon_032. Exists because a failed page render writes nothing anywhere else: Vercel logs it as a 200, Sentry only sees it if a boundary rendered, and every other monitor watches the database rather than the product.';
COMMENT ON TABLE public.gateway_retry_counts IS 'Reads the client library retried after a 504, per UTC day and HTTP method. A floor, not an exact count. Written by record_gateway_retry(). Not an alert.';
...
COMMENT ON TABLE public.suppression_reconciliation_snapshot IS 'Single-row verdict from /api/cron/suppression-reconcile, which reads the sending provider live for every prospect the send gate says must not be mailed. Read by mon_026.';

-- ═══════════════════════════════════════════════════════════════════════════
-- COVERAGE, counted against the live catalog at generation time
--
--     tables      50     views        32     functions   23
--     triggers    12     policies     52     indexes     155
--     constraints 239    sequences    2      extensions  7
--     comments    54
--
-- ═══════════════════════════════════════════════════════════════════════════
