...
async function main() {
  console.log('\n=== PHASE 4 RUN (REAL) ===\n')
  console.log('START: orchestrator run')
  ...
  try {
    ...
    if (runs?.length) {
      console.log(`  id: ${runs[0].id}`)
      console.log(`  status: ${runs[0].status}`)
      console.log(`  started_at: ${runs[0].started_at}`)
      console.log(`  completed_at: ${runs[0].completed_at || 'NULL'}`)
      console.log(`  duration_ms: ${runs[0].duration_ms || 'NULL'}`)
      if (runs[0].error_message) {
        console.log(`  error: ${runs[0].error_message}`)
      }
    }
    ...
    // ─── RECEIPT 6: Apollo credits consumed ────────────────────────────
    console.log('\nRECEIPT 6: Apollo credits consumed')
    console.log(`  estimated (~1 per enriched): ${enrichedCount}`)
